...
te_ICI30e30m = sum(en_ICI30m30e(1:42/dt)) * dt /(50-10); 
te_ICI30m30m = sum(en_ICI30m30m(1:42/dt)) * dt/(42-10); 
te_ICI30l30m = sum(en_ICI30m30l(1:42/dt)) * dt/(34-10); 
te_CCI30e30m = sum(en_CCI30m30e(1:42/dt)) * dt/(50-10); 
te_CCI30m30m = sum(en_CCI30m30m(1:42/dt)) * dt/(42-10); 
te_CCI30l30m = sum(en_CCI30m30l(1:42/dt)) * dt/(34-10); 
te_GWP30e    = sum(en_GWP30m(1:42/dt)   ) * dt/(50-10);
te_GWP30m    = sum(en_GWP30m(1:42/dt)   ) * dt/(42-10);
te_GWP30l    = sum(en_GWP30m(1:42/dt)   ) * dt/(34-10);
te_ICI45e45m = sum(en_ICI45m45e(1:85/dt)) * dt/(100-10); 
te_ICI45m45m = sum(en_ICI45m45m(1:85/dt)) * dt/(75-10); 
te_ICI45l45m = sum(en_ICI45m45l(1:85/dt)) * dt/(60-10); 
te_CCI45e45m = sum(en_CCI45m45e(1:85/dt)) * dt/(100-10); 
te_CCI45m45m = sum(en_CCI45m45m(1:85/dt)) * dt/(75-10); 
te_CCI45l45m = sum(en_CCI45m45l(1:85/dt)) * dt/(60-10); 
te_GWP45e    = sum(en_GWP45m(1:85/dt)   ) * dt/(100-10);
te_GWP45m    = sum(en_GWP45m(1:85/dt)   ) * dt/(75-10);
te_GWP45l    = sum(en_GWP45m(1:85/dt)   ) * dt/(60-10);

ov_ICI30e30m = max(rf_ICI30m30e(1:50/dt)) - 3; 
ov_ICI30m30m = max(rf_ICI30m30m(1:42/dt)) - 3; 
ov_ICI30l30m = max(rf_ICI30m30l(1:34/dt)) - 3; 
ov_CCI30e30m = max(rf_CCI30m30e(1:50/dt)) - 3; 
ov_CCI30m30m = max(rf_CCI30m30m(1:42/dt)) - 3; 
ov_CCI30l30m = max(rf_CCI30m30l(1:34/dt)) - 3; 
ov_GWP30e    = max(rf_GWP30m(1:50/dt)   ) - 3;
ov_GWP30m    = max(rf_GWP30m(1:42/dt)   ) - 3;
ov_GWP30l    = max(rf_GWP30m(1:34/dt)   ) - 3;
ov_ICI45e45m = max(rf_ICI45m45e(1:100/dt)) - 4.5; 
ov_ICI45m45m = max(rf_ICI45m45m(1:75/dt) ) - 4.5; 
ov_ICI45l45m = max(rf_ICI45m45l(1:60/dt) ) - 4.5; 
ov_CCI45e45m = max(rf_CCI45m45e(1:100/dt)) - 4.5; 
ov_CCI45m45m = max(rf_CCI45m45m(1:75/dt) ) - 4.5; 
ov_CCI45l45m = max(rf_CCI45m45l(1:60/dt) ) - 4.5; 
ov_GWP45e    = max(rf_GWP45m(1:100/dt)   ) - 4.5;
ov_GWP45m    = max(rf_GWP45m(1:75/dt)    ) - 4.5;
ov_GWP45l    = max(rf_GWP45m(1:60/dt)    ) - 4.5;

zero_vec30e  = zeros(50/dt,1);
zero_vec30m  = zeros(42/dt,1);
zero_vec30l  = zeros(34/dt,1);
zero_vec45e  = zeros(100/dt,1);
zero_vec45m  = zeros(75/dt,1);
zero_vec45l  = zeros(60/dt,1);

io_ICI30e30m = sum(max(rf_ICI30e30m(1:50/dt) - 3,zero_vec30e));
io_ICI30m30m = sum(max(rf_ICI30m30m(1:42/dt) - 3,zero_vec30m));
io_ICI30l30m = sum(max(rf_ICI30l30m(1:34/dt) - 3,zero_vec30l));
io_CCI30e30m = sum(max(rf_CCI30e30m(1:50/dt) - 3,zero_vec30e));
io_CCI30m30m = sum(max(rf_CCI30m30m(1:42/dt) - 3,zero_vec30m));
io_CCI30l30m = sum(max(rf_CCI30l30m(1:34/dt) - 3,zero_vec30l));
io_GWP30e    = sum(max(rf_GWP30e(1:50/dt)    - 3,zero_vec30e));
io_GWP30m    = sum(max(rf_GWP30m(1:42/dt)    - 3,zero_vec30m));
io_GWP30l    = sum(max(rf_GWP30l(1:34/dt)    - 3,zero_vec30l));

io_ICI45e45m = sum(max(rf_ICI45e45m(1:100/dt) - 4.5,zero_vec45e));
io_ICI45m45m = sum(max(rf_ICI45m45m(1:75/dt)  - 4.5,zero_vec45m));
io_ICI45l45m = sum(max(rf_ICI45l45m(1:60/dt)  - 4.5,zero_vec45l));
io_CCI45e45m = sum(max(rf_CCI45e45m(1:100/dt) - 4.5,zero_vec45e));
io_CCI45m45m = sum(max(rf_CCI45m45m(1:75/dt)  - 4.5,zero_vec45m));
io_CCI45l45m = sum(max(rf_CCI45l45m(1:60/dt)  - 4.5,zero_vec45l));
io_GWP45e    = sum(max(rf_GWP45e(1:100/dt)    - 4.5,zero_vec45e));
io_GWP45m    = sum(max(rf_GWP45m(1:75/dt)     - 4.5,zero_vec45m));
io_GWP45l    = sum(max(rf_GWP45l(1:60/dt)     - 4.5,zero_vec45l));
