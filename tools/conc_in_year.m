...
%=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=%
% Compute e(t) = e(0)*exp[ int_0^t g(t') dt' ].
%evec = e0*cumprod(1 + rate*dt);
evec = e0*exp( [0; cumsum(rate(1:end - 1)*dt)] ); %<-- It turns out this method is more accurate when rate is large.
